...
//                     <label className="w-[128px] h-[128px] border-2 border-[#A16A99] border-dashed rounded-lg p-[13.5px] flex items-center justify-center cursor-pointer bg-white hover:bg-gray-100 transition-colors">
//                       <input
//                         type="file"
//                         accept="image/*"
...
const ValuePropositionSection = () => {
  ...
  // Main form text inputs change handler
  const handleChange = (e) => {
    setFormData({
      ...formData,
      [e.target.name]: e.target.value,
  ...
  // Value proposition forms input change
  const handleChangeForm = (index, e) => {
    const newForms = [...forms];
    newForms[index][e.target.name] = e.target.value;
    setForms(newForms);
  ...
  // Add new value proposition entry
  const handleAddForm = () => {
    if (forms.length < maxForms) {
      setForms([...forms, { title: '', description: '', avatars: [] }]);
    }
  ...
  // Avatar upload handler for value proposition entries
  const handleAvatarUpload = (formIndex, event) => {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
  ...
  // Remove avatar from value proposition entry
  const removeAvatar = (formIndex, avatarIndex) => {
    const newForms = [...forms];
    newForms[formIndex].avatars = newForms[formIndex].avatars.filter(
      (_, i) => i !== avatarIndex
    );
    setForms(newForms);
  };

  // Save button handler - save all current form data & value props
  const handleSave = () => {
    setSavedData({
      mainForm: formData,
      valueProps: forms,
  ...
  // Discard button handler - reset everything
  const handleDiscard = () => {
    setFormData({
      title: '',
      description: '',
  ...
  return (
    <div className="w-[720px] bg-white rounded-lg shadow-md text-black border border-[#D0D5DD] p-6">
      <h2 className="text-lg text-[#101828] font-semibold mb-4 border-b border-[#EAECF0] pb-4">
        Value Proposition Section
      </h2>
        ...
        {forms.map((form, index) => (
          <div key={index} className="mb-4">
            <div className="bg-[#f9fafb] border border-[#D0D5DD] rounded">
              ...
              <div className="p-4">
                <p className="text-sm font-medium text-[#344054] pb-2">Icon</p>
                <div className="flex space-x-4 pb-6">
                  {form.avatars.length < maxAvatars && (
                    <label className="w-[128px] h-[128px] border-2 border-[#A16A99] border-dashed rounded-lg p-3 flex items-center justify-center cursor-pointer bg-white hover:bg-gray-100 transition-colors">
                      <input
                        type="file"
                        accept="image/*"
                        onChange={(e) => handleAvatarUpload(index, e)}
      ...
      <div className="flex justify-end space-x-4 border-t border-[#EAECF0] py-4">
      ...
      {/* Show saved data below */}
      {savedData && (
        <div className="mt-8 p-4 border border-gray-300 rounded bg-gray-50">
          <h3 className="text-xl font-semibold mb-4">Saved Data Preview</h3>

          <div className="mb-4">
            {/* <p><strong>Text:</strong> {savedData.mainForm.text}</p>
            <p><strong>Subtitle:</strong> {savedData.mainForm.subtitle}</p> */}
            <p><strong>CTA Text:</strong> {savedData.mainForm.ctaText}</p>
            <p><strong>CTA Link:</strong> {savedData.mainForm.ctaLink}</p>
          </div>

          <div>
            <h4 className="font-semibold mb-2">Value Proposition Entries:</h4>
            {savedData.valueProps.length === 0 && <p>No entries added.</p>}
            {savedData.valueProps.map((entry, i) => (
              <div key={i} className="mb-4 p-3 border rounded bg-white">
                <p><strong>Title:</strong> {entry.title}</p>
                <p><strong>Description:</strong> {entry.description}</p>
                <div className="flex space-x-2 mt-2">
                  {entry.avatars.map((avatar, idx) => (
                    <img
                      key={idx}
                      src={avatar}
                      alt={`Entry ${i + 1} Avatar ${idx + 1}`}
