...
const ImageUploader = ({
  ...
  id,
  containerClassName = '',
  dropText = 'Click to upload<br />or drag and drop<br />(Max: 480x480px)',
  maxDimensions = { width: 480, height: 480 },
}) => {
  ...
  const handleFile = (file) => {
    ...
    img.onload = () => {
      if (img.width <= maxDimensions.width && img.height <= maxDimensions.height) {
        ...
        alert(`Image must be max ${maxDimensions.width}x${maxDimensions.height} pixels.`);
      }
    };
  ...
  return (
    <div className="flex flex-col">
      <label
        htmlFor={id}
        className="text-sm font-medium text-[#344054] mb-2"
      >
        {label}
      </label>
      ...
      {!image ? (
        <div
          ...
          className={`border-2 border-[#A16A99] border-dashed rounded-lg p-[13.5px] flex items-center justify-center cursor-pointer hover:bg-gray-100 transition-colors ${containerClassName}`}
          role="button"
          tabIndex={0}
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center text-center">
            ...
            {/* Upload Text */}
            <div
              className="text-xs text-[#475467] font-normal"
              dangerouslySetInnerHTML={{ __html: dropText }}
            />
          </div>
        </div>
      ) : (
        <div className={`relative ${containerClassName}`}>
          ...
          <button
            onClick={removeImage}
            aria-label={`Remove uploaded image for ${label}`}
