...
const MixedSection = () => {
    ...
    const [avatarsWF, setavatarsWF] = useState([]);
    const maxAvatarsWF = 3;

    const handleFileDropWF = (event) => {
        event.preventDefault();
        const file = event.dataTransfer.files[0];
        if (file && avatarsWF.length < maxAvatarsWF) {
            const reader = new FileReader();
            reader.onload = (e) => {
            setavatarsWF([...avatarsWF, e.target.result]);
            };
            reader.readAsDataURL(file);
        }
    };

    const handleDragOverWF = (event) => {
        event.preventDefault();
    };

    const handleAvatarUploadWF = (event) => {
    ...
    const removeAvatarWF = (index) => {
        setavatarsWF(avatarsWF.filter((_, i) => i !== index));
    };
  ...
  return (
    <div className='w-[720px] bg-white rounded-lg shadow-md text-black border-1 border-[#D0D5DD]'>
      ...
      <div className='mb-6 px-6'>
        <div className="border-t-1 border-[#D0D5DD] pt-[16px]">
            ...
            <div className='mt-2'>
                <div className="space-y-[20px] mt-[16px]">
                    <InputField
                        label="Tab and Badge Label"
                        name="tabAndBadgeLabel"
                        placeholder="Enter label here"
                        value={formData.tabAndBadgeLabel}
            ...
            <h3 className="text-lg font-medium mb-2 mt-[20px] text-[#101828]">
          Card Entries ({forms.length} of {maxForms})
        </h3>
                    ...
                    <label className="w-[256px] h-[144px] border-2 border-[#A16A99] border-dashed rounded-lg p-[13.5px] flex items-center justify-center cursor-pointer hover:bg-gray-100 transition-colors">
                      ...
                      <div className="text-xs text-[#475467] font-normal flex flex-col items-center">
                        <div className='text-sm text-[#475467] font-normal '>
                            <span className="text-[#521749] text-sm font-semibold pr-1">Click to upload</span>
                        or drag and drop
                        </div>
                      </div>
                    </label>
                  ...
                  {form.avatars.map((avatar, avatarIndex) => (
                    <div key={avatarIndex} className="relative">
                      <img
                        src={avatar}
                        alt={`Avatar ${avatarIndex + 1}`}
                        className="w-[256px] h-[144px] rounded-lg object-cover"
                      />
                      <button
                        onClick={() => removeAvatar(index, avatarIndex)}
                        className="absolute top-[8px] right-[8px] cursor-pointer flex items-center justify-center"
